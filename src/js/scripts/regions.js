        ...
        popUpDiv.addEventListener('click', async (event) => {
            if (event.target.classList.contains('submit')) {
                ...
                if (tgtTextCnt == 'Agregar región') {
                    url = 'https://proyecto-data-warehouse.herokuapp.com/regions';
                    fetchMethod = 'POST';
                    fetchBody = JSON.stringify({ name: tgtName });
                }
                /*----------paises----------*/
                else if (tgtTextCnt == 'Agregar país') {
                    url = 'https://proyecto-data-warehouse.herokuapp.com/countries';
                    fetchMethod = 'POST';
                    fetchBody = JSON.stringify({ region_id: idBtn, name: tgtName });
                }else if (tgtTextCnt == 'Modificar país') {
                    url = `https://proyecto-data-warehouse.herokuapp.com/countries/${idBtn}`
                    fetchMethod = 'PUT';
                    fetchBody = JSON.stringify({ name: tgtName});
                ...
                /*----------ciudades----------*/
                    ...
                    url = 'https://proyecto-data-warehouse.herokuapp.com/cities';
                    fetchMethod = 'POST';
                    fetchBody = JSON.stringify({ countries_id: idBtn, name: tgtName });
                }
                else if (tgtTextCnt == 'Modificar ciudad') {
                    url = `https://proyecto-data-warehouse.herokuapp.com/cities/${idBtn}`
                    fetchMethod = 'PUT';
                    fetchBody = JSON.stringify({ name: tgtName});
