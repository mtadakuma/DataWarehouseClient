...
class Users {
    async getUsers() {
        try {
            ...
            let result = await fetch('https://proyecto-data-warehouse.herokuapp.com/users', {
                headers,
            });
    ...
    async submitUser() {
        submitPopUp.addEventListener('click', async () => {
            ...
            if (submitPopUp.textContent == 'Agregar') {
                url = 'https://proyecto-data-warehouse.herokuapp.com/users';
                fetchMethod = 'POST';
                ...
                url = `https://proyecto-data-warehouse.herokuapp.com/users/${idParam}`;
                fetchMethod = 'PUT';
                fetchBody = JSON.stringify({ first_name, last_name, email, isAdmin, password, username });
